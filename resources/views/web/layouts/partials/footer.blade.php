<div class="bg-secondary text-dark mt-5 pt-5">
    ...
    <div class="row">
        <div class="col-lg-4 col-md-12 mb-4 pe-3 pe-xl-5">
            ...
            <p class="mb-2"><i
                    class="fa fa-map-marker-alt text-primary me-3"></i>{{ ($siteSettings->contact_address ?? null) }}</p>
            <p class="mb-2"><i class="fa fa-envelope text-primary me-3"></i>{{ ($siteSettings->contact_email ?? null) }}</p>
            <p class="mb-0"><i class="fa fa-phone-alt text-primary me-3"></i>{{ ($siteSettings->contact_phone ?? null) }}</p>
            
            <div class="d-flex mt-4">
                @if(!empty($siteSettings->social_facebook) && $siteSettings->social_facebook != '#')
                    <a class="text-dark mb-2" href="{{ $siteSettings->social_facebook }}" target="_blank"><i class="fab fa-facebook-f text-primary me-3 fs-5"></i></a>
                @endif
                @if(!empty($siteSettings->social_twitter) && $siteSettings->social_twitter != '#')
                    <a class="text-dark mb-2" href="{{ $siteSettings->social_twitter }}" target="_blank"><i class="fab fa-twitter text-primary me-3 fs-5"></i></a>
                @endif
                @if(!empty($siteSettings->social_linkedin) && $siteSettings->social_linkedin != '#')
                    <a class="text-dark mb-2" href="{{ $siteSettings->social_linkedin }}" target="_blank"><i class="fab fa-linkedin-in text-primary me-3 fs-5"></i></a>
                @endif
                @if(!empty($siteSettings->social_instagram) && $siteSettings->social_instagram != '#')
                    <a class="text-dark mb-2" href="{{ $siteSettings->social_instagram }}" target="_blank"><i class="fab fa-instagram text-primary me-3 fs-5"></i></a>
                @endif
                @if(!empty($siteSettings->social_youtube) && $siteSettings->social_youtube != '#')
                    <a class="text-dark mb-2" href="{{ $siteSettings->social_youtube }}" target="_blank"><i class="fab fa-youtube text-primary me-3 fs-5"></i></a>
                @endif
            </div>
        </div>
        <div class="col-lg-8 col-md-12">
            <div class="row">
                <div class="col-md-4 mb-4">
                    <h5 class="fw-bold text-dark mb-4">Company</h5>
                    <div class="">
                        @php
                        ...
                        @if($companyMenu)
                            @foreach($companyMenu->items as $item)
                                ...
                                <li class="nav-item list-unstyled {{ !$loop->last ? 'mb-2' : '' }}">
                                    <a class="text-dark mb-2" href="{{ $linkUrl }}"><i class="fa fa-angle-right me-2"></i>{{ $item->title }}</a>
                                </li>
                            @endforeach
                        @endif
                    </div>
                ...
                <div class="col-md-4 mb-4">
                    <h5 class="fw-bold text-dark mb-4">Support</h5>
                    <div class="">
                        @php
                ...
                <div class="col-md-4 mb-4">
                    <h5 class="fw-bold text-dark mb-4">Newsletter</h5>
                    <form action="{{ route('subscribe') }}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <input type="email" name="email" class="form-control border-0 py-2" placeholder="Your Email"
                                required="required" />
                            @error('email')
    ...
    <div class="row border-top border-light py-4">
        <div class="col-md-6 px-xl-0">
            <p class="mb-md-0 text-center text-md-start text-dark">
                {!! str_replace(
            ...
            @if(isset($paymentMethods) && $paymentMethods->count() > 0)
                @foreach($paymentMethods as $paymentMethod)
                    @if($paymentMethod->media)
                        <img class="img-fluid me-2" src="{{ Storage::url($paymentMethod->media->src) }}" alt="{{ $paymentMethod->name }}" style="max-height: 30px;" loading="lazy">
                    @endif
                @endforeach
            @else
