...
<div class="container-fluid mb-4 {{ request()->routeIs('root') ? '' : 'navbarShadow' }}">
    <div class="row border-top px-xl-5">
        <div class="col-lg-3 d-none d-lg-block" id="{{ request()->routeIs('root') ? '' : 'navbar-vertical-wrapper' }}">
            <a class="btn shadow-none d-flex align-items-center justify-content-between bg-primary w-100 categories-main-btn"
                data-toggle="collapse" href="#navbar-vertical" style="height: 65px; margin-top: -1px; padding: 0 30px;">
                <h6 class="m-0 text-dark transition-colors">Categories</h6>
                <i class="fa fa-angle-down text-dark transition-colors"></i>
            </a>
            <nav class="collapse {{ Request::is('/') ? 'show' : 'position-absolute' }} navbar navbar-vertical navbar-light align-items-start p-0 border border-top-0 border-bottom-0"
                ...
                <div class="navbar-nav w-100 custom-sidebar-nav" style="{{ request()->routeIs('root') ? 'height: 410px' : '' }}">
                    @foreach ($categories as $category)
                        <div class="nav-item dropdown">
                            <a href="{{ route('categoryProducts', $category?->slug) }}" class="nav-link custom-nav-link">
                                {{ $category->name }}
