            ...
            <div class="card">
                <div class="card-header pt-4 d-flex justify-content-between align-items-center">
                    ...
                    <select id="menu-selector" class="form-select w-auto">
                        @foreach($menus as $m)
                            <option value="{{ $m->id }}" data-action="{{ route('admin.menus.items.store', $m->id) }}" data-name="{{ $m->name }}" {{ $activeMenu && $activeMenu->id == $m->id ? 'selected' : '' }}>
                                {{ $m->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="card-body p-4">                    
                    @foreach($menus as $m)
                        <div class="menu-list-container" id="menu-list-{{ $m->id }}" style="display: {{ $activeMenu && $activeMenu->id == $m->id ? 'block' : 'none' }};">
                            <ul class="list-group">
                    ...
                    @endforeach
                    <small class="text-muted d-block mt-4"><i class="mdi mdi-information-outline"></i> <em>Note: Drag and drop features will be added soon. Currently sorting by order added.</em></small>
                </div>
            </div>
        </div>
                ...
                <div class="card-body p-4">
                    ...
                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                      <li class="nav-item" role="presentation">
                        <button class="nav-link active" id="system-tab" data-bs-toggle="tab" data-bs-target="#system" type="button" role="tab">System Pages</button>
                      </li>
                      <li class="nav-item" role="presentation">
                        <button class="nav-link" id="pages-tab" data-bs-toggle="tab" data-bs-target="#pages" type="button" role="tab">Dynamic Pages</button>
                      </li>
                      <li class="nav-item" role="presentation">
                      ...
                      <li class="nav-item" role="presentation">
                        <button class="nav-link" id="custom-tab" data-bs-toggle="tab" data-bs-target="#custom" type="button" role="tab">Custom Link</button>
                      </li>
                    </ul>
                    ...
                    <div class="tab-content mt-4">
                        ...
                        <div class="tab-pane active" id="system" role="tabpanel">
                            <form class="menu-add-form" action="{{ route('admin.menus.items.store', $activeMenu ? $activeMenu->id : 0) }}" method="POST">
                                @csrf
                                <input type="hidden" name="type" value="system">
                                <div class="mb-4">
                                    <label class="mb-2">Select System Page</label>
                                ...
                                <div class="mb-4">
                                    <label class="mb-2">Link Title (Optional)</label>
                                    <input type="text" name="title" class="form-control" placeholder="Leave empty to use default name">
                                </div>
                        ...
                        <div class="tab-pane" id="pages" role="tabpanel">
                            <form class="menu-add-form" action="{{ route('admin.menus.items.store', $activeMenu ? $activeMenu->id : 0) }}" method="POST">
                                ...
                                <div class="mb-4">
                                    <label class="mb-2">Link Title</label>
                                    <input type="text" name="title" class="form-control" required placeholder="Page Link">
                                </div>
                                <button type="submit" class="btn btn-primary btn-sm">Add to Menu</button>
                            </form>
                        </div>
                        ...
                        <div class="tab-pane" id="categories" role="tabpanel">
                            <form class="menu-add-form" action="{{ route('admin.menus.items.store', $activeMenu ? $activeMenu->id : 0) }}" method="POST">
                                ...
                                <div class="mb-4">
                                    <label class="mb-2">Link Title</label>
                                    <input type="text" name="title" class="form-control" required placeholder="Category Link">
                                </div>
                                <button type="submit" class="btn btn-primary btn-sm">Add to Menu</button>
                            </form>
                        </div>
                        ...
                        <div class="tab-pane" id="custom" role="tabpanel">
                            <form class="menu-add-form" action="{{ route('admin.menus.items.store', $activeMenu ? $activeMenu->id : 0) }}" method="POST">
                                ...
                                <div class="mb-4">
                                    <label class="mb-2">Link Title</label>
                                    <input type="text" name="title" class="form-control" required placeholder="Menu Link">
                                </div>
                                <button type="submit" class="btn btn-primary btn-sm">Add to Menu</button>
                            </form>
                        </div>
            ...
            <div class="modal-content">
                <form id="edit-menu-form" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3" id="edit-url-container" style="display: none;">
                            <label class="form-label">URL</label>
                            <input type="text" name="url" id="edit-url" class="form-control">
                        </div>
                    </div>
