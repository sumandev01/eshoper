...
@section('title', ($siteSettings->site_title ?? null) . ' - ' . 'Edit Courier')
@section('content')
                    ...
                    <div class="card-body p-4">
                        <form action="{{ route('admin.couriers.update', $courier->id) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="url" class="form-control @error('tracking_url') is-invalid @enderror" id="tracking_url" name="tracking_url" value="{{ old('tracking_url', $courier->tracking_url) }}" placeholder="e.g. https://sundarbancourier.com.bd/">
                            ...
                            <div class="mb-4">
                                ...
                                <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                    <option value="1" {{ old('status', $courier->status) == '1' ? 'selected' : '' }}>Active</option>
                                    <option value="0" {{ old('status', $courier->status) == '0' ? 'selected' : '' }}>Inactive</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
